...
function [landMark] = landMarkfunc(mode, clipNum, numMax)
    min = 1;
    max = 200;
    clipNumStr = int2str(clipNum);
    if(mode == 'valence')
        A = readmatrix(append('valence\dev_',clipNumStr,'.csv'));
    else
        A = readmatrix(append('arousal\dev_',clipNumStr,'.csv'));
    end
    time = A(:,1);
    Rator1 = A(:,2);
    Rator2 = A(:,3);
    ...
    A = intersect(landMarkTop_1, landMarkTop_2);
    GLM1 = intersect(A, landMarkTop_3);
    GLM1 = sort(GLM1);
    
    
    B = intersect(landMarkTop_4, landMarkTop_5);
    GLM2 = intersect(B, landMarkTop_6);
    GLM2 = sort(GLM2);
    
    GLM = intersect(GLM1,GLM2);
    finalList = []
    for i = 2:1:length(GLM)
        finalList = [finalList;(GLM(i)-3)];
        finalList = [finalList;(GLM(i)-2)];
        finalList = [finalList;(GLM(i)-1)];
        finalList = [finalList;(GLM(i))];
        finalList = [finalList;(GLM(i)+2)];
        finalList = [finalList;(GLM(i)+1)];
        finalList = [finalList;(GLM(i)+3)];
    end
